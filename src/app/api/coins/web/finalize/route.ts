...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sessionId) return safeJson({ ok: false, error: "Missing session_id" }, 400);
    ...
    const idempotencyKey =
      String(meta?.idempotency_key ?? "").trim() || String(pi?.metadata?.idempotency_key ?? "").trim() || `stripe:cs:${sessionId}`;
