...
export type GifterLevelInfo = {
  ...
  tierIndex: number;
  tierLetter: string;
  displayLevel: number; // standard: 1..50, diamond: 1..
  isDiamond: boolean;
  ringColor: string;
  tierStartUsd: number;
  tierEndUsd: number;
...
export function getGifterLevel(totalUsdRaw: number): GifterLevelInfo {
  ...
  if (totalUsd >= DIAMOND_ENTRY_USD) {
    // Diamond Elite Infinite tier
    // Curve: diamond level boundaries follow a power curve with exponent > 1
    // so each level costs more than the previous. This is invertible and cheap.
    // Anchors (approx): L10 ~= $1.1M, L500 ~= $8M.
    const L1 = 10;
    const E1 = 100_000; // excess above entry
    const L2 = 500;
    const E2 = 7_000_000; // excess above entry
  ...
  // Standard tiers (Grey..Obsidian), 50 levels each
  let tierIdx = 0;
